...
namespace Minio
{
    ...
    public partial class DefaultMinioClient : IMinioClient
    {
        ...
        public DefaultMinioClient(MinioSettings minioSettings)
        {
            this.Secure = minioSettings.Secure;
        ...
        public void SetCustomUserAgent(string customUserAgent)
        {
            ...
            {
                throw new ArgumentException($"{nameof(customUserAgent)} cannot be null or empty");
            }
        ...
        protected virtual SystemUserAgentSettings GetSystemUserAgentSettings()
        {
#if NETSTANDARD1_6 || NETSTANDARD1_1
            return new SystemUserAgentSettings
            {
                ModelArch = System.Runtime.InteropServices.RuntimeInformation.OSArchitecture.ToString(),
                ModelDescription = System.Runtime.InteropServices.RuntimeInformation.OSDescription,
                Platform = "Core",
                AppVersion = this.GetType().GetTypeInfo().Assembly.GetName().Version.ToString()
            };

#elif NET452
            return new SystemUserAgentSettings
            {
                ModelArch = System.Environment.Is64BitOperatingSystem ? "x86_64" : "x86",
                ModelDescription = System.Environment.OSVersion.ToString(),
                Platform = "NET452",
                AppVersion = this.GetType().GetTypeInfo().Assembly.GetName().Version.ToString()
            };
#else
        ...
        protected virtual CryptoProvider CreateCryptoProvider()
        {
            return new CryptoProvider();
        }

        protected virtual LogProvider CreateLogProvider()
        {
        ...
        private void InitClient(MinioSettings minioSettings)
        {
            ...
            if (Uri.TryCreate(this.BaseUrl, UriKind.Absolute, out var hostUri))
            {
                if (hostUri.Scheme == "https" || hostUri.Scheme == "https")
                {
                    throw new InvalidEndpointException("Endpoint cannot has scheme http|https");
                }
            }
            ...
            this.authenticator = new V4Authenticator(this);
