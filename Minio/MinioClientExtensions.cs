...
namespace Minio
{
    public static class MinioClientExtensions
    {
        ...
        public static MinioClient Build(this MinioClient minioClient)
        {
            ...
            if (minioClient.Provider != null && minioClient.Provider.GetType() != typeof(ChainedProvider) && minioClient.SessionToken == null)
                throw new MinioException("User Access Credentials Provider not initialized correctly.");
            if (minioClient.Provider == null && (string.IsNullOrEmpty(minioClient.AccessKey) || string.IsNullOrEmpty(minioClient.SecretKey)))
            ...
            minioClient.HttpClient ??= minioClient.Proxy is null ? new HttpClient() : new HttpClient(new HttpClientHandler { Proxy = minioClient.Proxy });
