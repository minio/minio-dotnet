    ...
    public class SelectResponseStream
    {
        ...
        private void Start()
        {
            ...
            while (_isProcessing)
            {
                ...
                var isValidPrelude =
                    Crc32.TryHash(inputArray.Slice(0, inputArray.Length - 4), destinationPrelude, out _);
                if (!isValidPrelude)
                {
                ...
                var isValidMessage =
                    Crc32.TryHash(inputArray.Slice(0, inputArray.Length - 4), destinationMessage, out _);
                if (!isValidMessage)
                {
                ...
                if (headerMap.TryGetValue(":event-type", out value))
                {
                    ...
                    if (value.Equals("Cont", StringComparison.OrdinalIgnoreCase) || payloadLength < 1)
                    {
                        continue;
                    }
